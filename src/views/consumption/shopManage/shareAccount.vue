    ...
    open (selectList) {
      ...
      if (selectList && selectList.length > 0) {
        for (let i = 0; i < selectList.length; i++) {
          this.selectList.push(selectList[i].shareUserId)
        }
      }
        ...
        getCityAdmin(params).then(({data})=>{
          ...
          for (let i = 0; i < list.length; i++) {
            if (list[i].userType == 3 || list[i].userType == 4) {
              list[i].shareHierarchy = '平台'
              list[i].cityId = 1
              list[i].cityName = '全国'
            } else if (list[i].userType == 1 || list[i].userType == 2) {
              list[i].shareHierarchy = '城市'
            } else if (list[i].userType == 5 || list[i].userType == 6) {
              list[i].shareHierarchy = '区县'
            } else {
              list[i].shareHierarchy = ''
            }
          }
          resolve({
