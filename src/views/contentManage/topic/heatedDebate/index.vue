    ...
    >
      ...
      <template slot="form">
        ...
        >
          ...
          <el-form-item v-if="publishType != 'view'">
            <!-- <el-button @click="handlePreview">预览</el-button> -->
            <el-button v-if="publishType == 'edit' && formData.state == '0' || publishType == 'add' && (!titleObj.heatedDebateId || titleObj.heatedDebateId === 0)" :loading="formLoading" @click="handleDraft"
              >保存草稿</el-button
            >
            <el-button :loading="formLoading" @click="handleSave"
              >直接发布</el-button
            >
          </el-form-item>
        </el-form>
...
export default {
  ...
  methods: {
    ...
    save(state) {
      if (this.publishType == 'edit' || !this.titleObj.heatedDebateId) {
        let formData = this.formData;
        formData.title = this.titleObj.title
        saveDebate({ ...formData, state })
        .then(({ data }) => {
          this.publish = false;
          this.$notify({
            title: "成功",
        ...
        for (let i = 0; i < this.formData.images.length; i++) {
          commentAnnexes.push({
            attachFile: this.formData.images[i],
            original: ''
          })
        }
        let formData = {
          ...
          type: 4,
          dataId: this.titleObj.heatedDebateId,
          commentAnnexes
        }
        // return
